...
async function capturePhoto(category, index) {
    ...
    // Flag untuk prevent duplikasi penyimpanan foto
    ...
    if (navigator.geolocation) {
        ...
        const processPhotoWithoutGPS = () => {
            if (photoSaved) return; // Prevent duplikasi
            photoSaved = true;
            ...
            try {
                ...
                const startY = canvas.height - (lineHeight * 3.5);
                ...
                ctx.font = `${fontSize}px Arial`;
                ctx.fillText(`⚠ GPS tidak tersedia`, padding, startY + (lineHeight * 3.5));
        ...
        const processPhotoWithGPS = async (lat, lng) => {
            ...
            try {
                ...
                const minutes = String(timestamp.getMinutes()).padStart(2, '0');
                const seconds = String(timestamp.getSeconds()).padStart(2, '0');
                const timeStr = `${hours}:${minutes}:${seconds} ${timezone}`;
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            function(error) {
                // Jika geolocation error, tunggu sebentar lalu retry
                console.warn('Geolocation failed:', error.message);
                infoDiv.innerHTML = '<p class="text-yellow-600">⏳ Mencoba akses GPS lagi...</p>';

                // Retry sekali sebelum fallback
                setTimeout(() => {
                    if (photoSaved) return; // Sudah tersimpan
                    navigator.geolocation.getCurrentPosition(
                        function(position) {
                            const lat = position.coords.latitude;
                            const lng = position.coords.longitude;
                        ...
                        function(retryError) {
                            // Jika tetap gagal, proses tanpa GPS
                        ...
                        },
                        {
                            ...
                            timeout: 3000,  // Retry dengan timeout lebih singkat
                            maximumAge: 0
                        }
                    );
            ...
            {
                enableHighAccuracy: true,
                timeout: 15000,  // Timeout 15 detik - lebih lama untuk tunggu GPS
                maximumAge: 0
            }
        );
    } else {
