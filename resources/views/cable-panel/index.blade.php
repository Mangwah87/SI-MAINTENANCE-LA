<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-6">
                        <h3 class="text-xl sm:text-2xl font-bold text-gray-800">Data PM Kabel & Panel</h3>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150 data-row">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex justify-center gap-2">
                                            {{-- Tombol Show --}}
                                            <a href="{{ route('cable-panel.show', $maintenance->id) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium rounded-md shadow-sm" title="Lihat Detail">
                                                <i data-lucide="eye" class="h-4 w-4"></i>
                                            </a>
                                            {{-- Tombol Edit --}}
                                            <a href="{{ route('cable-panel.edit', $maintenance->id) }}" class="inline-flex items-center px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-medium rounded-md shadow-sm" title="Edit">
                                                <i data-lucide="edit-3" class="h-4 w-4"></i>
                                            </a>
                                            {{-- Tombol PDF --}}
                                            <a href="{{ route('cable-panel.pdf', $maintenance->id) }}" class="inline-flex items-center px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs font-medium rounded-md shadow-sm" title="Download PDF" target="_blank">
                                                <i data-lucide="printer" class="h-4 w-4"></i>
                                            </a>
                                            {{-- Tombol Delete --}}
                                            <form action="{{ route('cable-panel.destroy', $maintenance->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr id="noDataRow">
                                    <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                                         <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm mt-1">Klik "Tambah Data" untuk menambahkan data PM Kabel & Panel</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
    ...
    @push('scripts')
    <script>
        // Script search (sama persis dengan grounding)
        // ... (Script dari file grounding index.blade.php Anda bisa dicopy-paste di sini) ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function filterTable() {
                ...
                if (noResultsRow.classList.contains('hidden') && dataRows.length === 0) {
                ...
                if (noDataRow) {
                 ...
                 if (!found && dataRows.length > 0 && !noDataRow) {
                    noResultsRow.classList.remove('hidden');
