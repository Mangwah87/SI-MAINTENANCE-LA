<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Formulir Tindak Lanjut Preventive Maintenance
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('tindak-lanjut.update', $tindakLanjut) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <!-- Berdasarkan (RADIO BUTTON) -->
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center">
                                    <input type="radio" name="berdasarkan" id="permohonan_tindak_lanjut" value="permohonan_tindak_lanjut"
                                           ...
                                           {{ old('berdasarkan', $tindakLanjut->permohonan_tindak_lanjut ? 'permohonan_tindak_lanjut' : ($tindakLanjut->pelaksanaan_pm ? 'pelaksanaan_pm' : '')) == 'permohonan_tindak_lanjut' ? 'checked' : '' }}>
                                ...
                                <div class="flex items-center">
                                    <input type="radio" name="berdasarkan" id="pelaksanaan_pm" value="pelaksanaan_pm"
                                           class="border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                           {{ old('berdasarkan', $tindakLanjut->permohonan_tindak_lanjut ? 'permohonan_tindak_lanjut' : ($tindakLanjut->pelaksanaan_pm ? 'pelaksanaan_pm' : '')) == 'pelaksanaan_pm' ? 'checked' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="date" name="tanggal" id="tanggal" required
                                       ...
                                       value="{{ old('tanggal', $tindakLanjut->tanggal ? $tindakLanjut->tanggal->format('Y-m-d') : '') }}">
                                @error('tanggal')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <!-- Status Penyelesaian (RADIO BUTTON) -->
                            ...
                            <div class="space-y-3">
                                <div class="flex items-start">
                                    <input type="radio" name="status_penyelesaian" id="selesai" value="selesai"
                                           ...
                                           {{ old('status_penyelesaian', $tindakLanjut->selesai ? 'selesai' : ($tindakLanjut->tidak_selesai ? 'tidak_selesai' : '')) == 'selesai' ? 'checked' : '' }}
                                           onchange="toggleStatusPenyelesaian(this.value)">
                                    <div class="ml-3 flex-1">
                                        <label for="selesai" class="text-sm text-gray-700">Selesai</label>
                                        <div id="selesai-fields" class="mt-2 grid grid-cols-2 gap-3" style="display: {{ old('status_penyelesaian', $tindakLanjut->selesai ? 'selesai' : '') == 'selesai' ? 'grid' : 'none' }};">
                                            <div>
                                                ...
                                                <input type="date" name="selesai_tanggal" id="selesai_tanggal"
                                                       ...
                                                       value="{{ old('selesai_tanggal', $tindakLanjut->selesai_tanggal ? $tindakLanjut->selesai_tanggal->format('Y-m-d') : '') }}">
                                ...
                                <div class="flex items-start">
                                    <input type="radio" name="status_penyelesaian" id="tidak_selesai" value="tidak_selesai"
                                           class="mt-1 border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                           {{ old('status_penyelesaian', $tindakLanjut->selesai ? 'selesai' : ($tindakLanjut->tidak_selesai ? 'tidak_selesai' : '')) == 'tidak_selesai' ? 'checked' : '' }}
                                           onchange="toggleStatusPenyelesaian(this.value)">
                                    <div class="ml-3 flex-1">
                                        <label for="tidak_selesai" class="text-sm text-gray-700">Tidak selesai, Tindak lanjut</label>
                                        <div id="tidak-selesai-fields" class="mt-2" style="display: {{ old('status_penyelesaian', $tindakLanjut->tidak_selesai ? 'tidak_selesai' : '') == 'tidak_selesai' ? 'block' : 'none' }};">
                        ...
                        <div class="mb-6 border-t pt-4">
                            ...
                            <div id="pelaksana-container" class="space-y-3">
                                @php
                                    // Prioritaskan old() data untuk validasi error
                                    $oldPelaksana = old('pelaksana');
                                    $pelaksanaData = $oldPelaksana ?: ($tindakLanjut->pelaksana ?: []);
                                    $pelaksanaCount = count($pelaksanaData);
                                @endphp

                                @if($pelaksanaCount > 0)
                                    @foreach($pelaksanaData as $index => $pelaksana)
                                        <div class="pelaksana-item border rounded-lg p-3 bg-gray-50">
                                            <div class="flex justify-between items-center mb-3">
                                                <h4 class="font-medium text-gray-700 text-sm">Pelaksana {{ $index + 1 }}</h4>
                                                @if($index > 0)
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                                <div>
                                                    ...
                                                    <input type="text" name="pelaksana[{{ $index }}][nama]" 
                                                           class="w-full rounded-md border-gray-300 shadow-sm text-sm" 
                                                           value="{{ is_array($pelaksana) ? ($pelaksana['nama'] ?? '') : '' }}" 
                                                           required>
                                                    @error('pelaksana.'.$index.'.nama')
                                                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                                    @enderror
                                                </div>
                                                <div>
                                                    <label class="block text-xs font-medium text-gray-700 mb-1">Department <span class="text-red-500">*</span></label>
                                                    <input type="text" name="pelaksana[{{ $index }}][department]" 
                                                           class="w-full rounded-md border-gray-300 shadow-sm text-sm" 
                                                           value="{{ is_array($pelaksana) ? ($pelaksana['department'] ?? '') : '' }}" 
                                                           required>
                                                    @error('pelaksana.'.$index.'.department')
                                                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                                    @enderror
                                                </div>
                                                <div>
                                                    <label class="block text-xs font-medium text-gray-700 mb-1">Sub Department</label>
                                                    <input type="text" name="pelaksana[{{ $index }}][sub_department]" 
                                                           class="w-full rounded-md border-gray-300 shadow-sm text-sm" 
                                                           value="{{ is_array($pelaksana) ? ($pelaksana['sub_department'] ?? '') : '' }}">
                                                    @error('pelaksana.'.$index.'.sub_department')
                                                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                                    @enderror
                                                </div>
                                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="mengetahui_nama" id="mengetahui_nama" required
                                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                           value="{{ old('mengetahui_nama', $tindakLanjut->mengetahui ? $tindakLanjut->mengetahui['nama'] : '') }}">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="mengetahui_nik" id="mengetahui_nik" 
                                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                           value="{{ old('mengetahui_nik', $tindakLanjut->mengetahui ? $tindakLanjut->mengetahui['nik'] : '') }}">
    ...
    @push('scripts')
    <script>
        // Initialize pelaksanaIndex dengan jumlah pelaksana yang sudah ada
        let pelaksanaIndex = {{ $pelaksanaCount > 0 ? $pelaksanaCount : 1 }};
