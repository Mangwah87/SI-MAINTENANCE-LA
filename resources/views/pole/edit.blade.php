                    ...
                    <form action="{{ route('pole.update', $pole->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                    ...
                                    <select name="central_id" id="central_id" required
                                        ...
                                        @foreach($centrals as $central)
                                            <option value="{{ $central->id }}"
                                                    {{ old('central_id', $pole->central_id) == $central->id ? 'selected' : '' }}>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <select name="type_pole" required
                                        ...
                                        <option value="SST" {{ old('type_pole', $pole->type_pole) == 'SST' ? 'selected' : '' }}>SST</option>
                                        <option value="Pole" {{ old('type_pole', $pole->type_pole) == 'Pole' ? 'selected' : '' }}>Pole</option>
                                        <option value="Tripole" {{ old('type_pole', $pole->type_pole) == 'Tripole' ? 'selected' : '' }}>Tripole</option>
                                        <option value="Triangle" {{ old('type_pole', $pole->type_pole) == 'Triangle' ? 'selected' : '' }}>Triangle</option>
                                        <option value="Triangle Wired" {{ old('type_pole', $pole->type_pole) == 'Triangle Wired' ? 'selected' : '' }}>Triangle Wired</option>
                                    </select>
                                        ...
                                        <div class="space-y-3">
                                            <div>
                                                ...
                                                <input type="text" name="{{ $check[0] }}"
                                                       value="{{ old($check[0], $pole->{$check[0]}) }}"
                                                    ...
                                                    @foreach(['OK', 'NOK'] as $status)
                                                        <label class="inline-flex items-center cursor-pointer">
                                                            <input type="radio" name="status_{{ $check[0] }}" value="{{ $status }}"
                                                                   {{ old('status_'.$check[0], $pole->{'status_'.$check[0]}) == $status ? 'checked' : '' }}
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            <input type="text" name="inclination_measurement"
                                                   value="{{ old('inclination_measurement', $pole->inclination_measurement) }}"
                                                ...
                                                @foreach(['OK', 'NOK'] as $status)
                                                    <label class="inline-flex items-center cursor-pointer">
                                                        <input type="radio" name="status_inclination_measurement" value="{{ $status }}"
                                                               {{ old('status_inclination_measurement', $pole->status_inclination_measurement) == $status ? 'checked' : '' }}
                                                               class="form-radio text-blue-600 focus:ring-blue-500">
                                                        <span class="ml-2 text-sm sm:text-base text-gray-700">{{ $status }}</span>
                                                    </label>
                                                @endforeach
                                        ...
                                        <div class="space-y-3">
                                            <div>
                                                ...
                                                <input type="text" name="executor_{{ $i }}"
                                                       value="{{ old('executor_'.$i, $pole->{'executor_'.$i}) }}"
                                            ...
                                            <div>
                                                ...
                                                <select name="mitra_internal_{{ $i }}"
                                                    ...
                                                    <option value="Mitra" {{ old('mitra_internal_'.$i, $pole->{'mitra_internal_'.$i}) == 'Mitra' ? 'selected' : '' }}>Mitra</option>
                                                    <option value="Internal" {{ old('mitra_internal_'.$i, $pole->{'mitra_internal_'.$i}) == 'Internal' ? 'selected' : '' }}>Internal</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            <input type="text" name="verifikator"
                                                   value="{{ old('verifikator', $pole->verifikator) }}"
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="verifikator_nik"
                                                   value="{{ old('verifikator_nik', $pole->verifikator_nik) }}"
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            <input type="text" name="head_of_sub_department"
                                                   value="{{ old('head_of_sub_department', $pole->head_of_sub_department) }}"
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="head_of_sub_department_nik"
                                                   value="{{ old('head_of_sub_department_nik', $pole->head_of_sub_department_nik) }}"
                        ...
                        <div class="mb-6 sm:mb-8">
                            ...
                            <textarea name="notes" rows="3"
                                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm sm:text-base"
                                      placeholder="Tambahkan catatan atau informasi tambahan di sini...">{{ old('notes', $pole->notes) }}</textarea>
                        </div>
