    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('central.update', $central->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select id="area" name="area"
                                ...
                                <option value="BALI" {{ old('area', $central->area) == 'BALI' ? 'selected' : '' }}>
                                    BALI</option>
                                <option value="NTT" {{ old('area', $central->area) == 'NTT' ? 'selected' : '' }}>NTT
                                </option>
                                <option value="NTB" {{ old('area', $central->area) == 'NTB' ? 'selected' : '' }}>NTB
                                </option>
                            </select>
                            @error('area')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
