...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Inventory Device</h1>
    </div>

    <form action="{{ route('inventory-device.update', $inventory->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div>
                ...
                <select name="central_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2">
                    ...
                    @foreach($centrals as $central)
                        <option value="{{ $central->id }}" {{ $inventory->central_id == $central->id ? 'selected' : '' }}>
                            {{ $central->nama }} - {{ $central->area }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <input type="date" name="date" required
                       value="{{ $inventory->date->format('Y-m-d') }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Time</label>
                <input type="time" name="time" required
                       value="{{ \Carbon\Carbon::parse($inventory->time)->format('H:i') }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full border-collapse border border-gray-300">
                    ...
                    <tbody id="deviceSentralTable">
                        ...
                        @endphp
                        @if(!empty($deviceSentral))
                            @foreach($deviceSentral as $index => $device)
                            <tr>
                                <td class="border border-gray-300 px-2 py-2 text-center text-sm">{{ $index + 1 }}</td>
                                <td class="border border-gray-300 px-2 py-1">
                                    <input type="text" name="device_sentral[{{ $index + 1 }}][equipment]"
                                           value="{{ $device['equipment'] ?? '' }}"
                                           class="w-full px-2 py-1 text-sm border-0 focus:ring-0" required>
                                </td>
                                <td class="border border-gray-300 px-2 py-1">
                                    <input type="number" name="device_sentral[{{ $index + 1 }}][qty]"
                                           value="{{ $device['qty'] ?? '' }}"
                                           class="w-full px-2 py-1 text-sm border-0 focus:ring-0" min="0">
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="device_sentral[{{ $index + 1 }}][status_active]" value="1"
                                           {{ isset($device['status_active']) && $device['status_active'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="device_sentral[{{ $index + 1 }}][status_shutdown]" value="1"
                                           {{ isset($device['status_shutdown']) && $device['status_shutdown'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="device_sentral[{{ $index + 1 }}][bonding_connect]" value="1"
                                           {{ isset($device['bonding_connect']) && $device['bonding_connect'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="device_sentral[{{ $index + 1 }}][bonding_not_connect]" value="1"
                                           {{ isset($device['bonding_not_connect']) && $device['bonding_not_connect'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1">
                                    <input type="text" name="device_sentral[{{ $index + 1 }}][keterangan]"
                                           value="{{ $device['keterangan'] ?? '' }}"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full border-collapse border border-gray-300">
                    ...
                    <tbody id="supportingFacilitiesTable">
                        ...
                        @endphp
                        @if(!empty($supportingFacilities))
                            @foreach($supportingFacilities as $index => $device)
                            <tr>
                                <td class="border border-gray-300 px-2 py-2 text-center text-sm">{{ $index + 1 }}</td>
                                <td class="border border-gray-300 px-2 py-1">
                                    <input type="text" name="supporting_facilities[{{ $index + 1 }}][equipment]"
                                           value="{{ $device['equipment'] ?? '' }}"
                                           class="w-full px-2 py-1 text-sm border-0 focus:ring-0" required>
                                </td>
                                <td class="border border-gray-300 px-2 py-1">
                                    <input type="number" name="supporting_facilities[{{ $index + 1 }}][qty]"
                                           value="{{ $device['qty'] ?? '' }}"
                                           class="w-full px-2 py-1 text-sm border-0 focus:ring-0" min="0">
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="supporting_facilities[{{ $index + 1 }}][status_active]" value="1"
                                           {{ isset($device['status_active']) && $device['status_active'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="supporting_facilities[{{ $index + 1 }}][status_shutdown]" value="1"
                                           {{ isset($device['status_shutdown']) && $device['status_shutdown'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="supporting_facilities[{{ $index + 1 }}][bonding_connect]" value="1"
                                           {{ isset($device['bonding_connect']) && $device['bonding_connect'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
                                    <input type="checkbox" name="supporting_facilities[{{ $index + 1 }}][bonding_not_connect]" value="1"
                                           {{ isset($device['bonding_not_connect']) && $device['bonding_not_connect'] ? 'checked' : '' }}>
                                </td>
                                <td class="border border-gray-300 px-2 py-1">
                                    <input type="text" name="supporting_facilities[{{ $index + 1 }}][keterangan]"
                                           value="{{ $device['keterangan'] ?? '' }}"
                                           class="w-full px-2 py-1 text-sm border-0 focus:ring-0">
                                </td>
                                <td class="border border-gray-300 px-2 py-1 text-center">
        ...
        <div class="mb-6">
            ...
            <textarea name="notes" rows="3"
                      class="w-full border border-gray-300 rounded-lg px-3 py-2">{{ $inventory->notes }}</textarea>
        </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="flex gap-2">
                    <input type="text" name="executor_{{ $i }}" placeholder="Executor {{ $i }} Name"
                           value="{{ $inventory->{'executor_'.$i} }}"
                           class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm">
                    <select name="mitra_internal_{{ $i }}" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
                        <option value="">-</option>
                        <option value="Mitra" {{ $inventory->{'mitra_internal_'.$i} == 'Mitra' ? 'selected' : '' }}>Mitra</option>
                        <option value="Internal" {{ $inventory->{'mitra_internal_'.$i} == 'Internal' ? 'selected' : '' }}>Internal</option>
                    </select>
                </div>
                @endfor
            </div>
        ...
        <div class="mb-6 grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="head_of_sub_department"
                       value="{{ $inventory->head_of_sub_department }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 mb-2">
                <input type="text" name="head_of_sub_department_nik" placeholder="NIK"
                       value="{{ $inventory->head_of_sub_department_nik }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="file" name="images[]" multiple accept="image/*"
                   class="w-full border border-gray-300 rounded-lg px-3 py-2">
            <p class="text-xs text-gray-500 mt-1">Leave empty to keep existing images</p>
        </div>
...
<script>
let deviceSentralCounter = {{ !empty($deviceSentral) ? count($deviceSentral) : 0 }};
let supportingFacilityCounter = {{ !empty($supportingFacilities) ? count($supportingFacilities) : 0 }};
